...
namespace Client
{
	...
	std::shared_ptr<MeshRenderer> MeshRendererParser::Parse(JsonObject& p_data)
	{
		...
		builder.SetSubMeshes(RessourceManager::FindMeshes(uuids::uuid::from_string(meshUuid).value()));
		...
		if (materialsUuidList != p_data)
		{
			for (auto& materialUuid : materialsUuidList)
			{
				...
				materials.push_back(RessourceManager::FindMaterial(uuids::uuid::from_string(std::string(materialUuid)).value()));
			}
		}
