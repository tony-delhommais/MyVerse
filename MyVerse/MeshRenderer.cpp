...
namespace Client
{
	...
	std::shared_ptr<MeshRenderer> MeshRenderer::Make(JsonObject& p_componentProprety)
	{
		...
		auto uuid = uuids::uuid::from_string(mesh_uuid).value();
		...
		if (materialParametersList != p_componentProprety)
		{
			for (auto& materialParameters : materialParametersList)
			{
				...
				uuid = uuids::uuid::from_string(material_uuid).value();

				meshRenderer->m_usedMaterialsUuid.insert(std::map<int, uuids::uuid>::value_type(linkId, uuid));
			}
		}
