...
namespace Client
{
	...
	Entity::Entity(bool p_isActive, const std::string& p_tag, const uuids::uuid& p_prefabReferenceUuid, std::shared_ptr<Transform> p_localTransform, std::shared_ptr<Camera> p_camera, std::shared_ptr<MeshRenderer> p_meshRenderer, std::list<std::shared_ptr<Script>> p_scripts) :
		m_tag(p_tag),
	...
	std::shared_ptr<Entity> Entity::InstantiatePrefab(const uuids::uuid& p_prefabUuid)
	{
	...
	void Entity::SetPrefabReferenceUuid(const uuids::uuid& p_prefabReferenceUuid)
	{
		m_prefabReferenceUuid = p_prefabReferenceUuid;
	}
